...
const OverviewSeaPassengersStatusCardProvince: React.FC<any> = ({cityTitle}) => {
  ...
  const getPassengerVaccinateInfo = async (province: any) => {
    setLoading(true);
    try {
      const res = await passengerService.getDoses({province, type: 'SHIP'}, {cancelToken: source.token});
      if (res.status === 200) {
  ...
  const getTripCount = async (province: any) => {
    setTripLoading(true);
    try {
      const {data} = await hcsService.tripsCount({province, type: 'SHIP'}, {cancelToken: source.token});
