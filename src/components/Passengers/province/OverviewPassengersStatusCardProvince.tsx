...
const OverviewPassengerStatusCardProvince: React.FC<any> = ({cityTitle}) => {
  ...
  const getPcrResult = async (province: any): Promise<any> => {
    setPcrLoading(true);
    try {
      const res = await passengerService.passengerTestResult({province}, {cancelToken: source.token});
      if (res.status === 200) {
  ...
  const getPassengerVaccinateInfo = async (province: any) => {
    setLoading(true);
    try {
      const res = await passengerService.getDoses({province}, {cancelToken: source.token});
      if (res.status === 200) {
  ...
  const getTripCount = async (province: any) => {
    setTripLoading(true);
    try {
      const {data} = await hcsService.tripsCount({province}, {cancelToken: source.token});
  ...
  return (
    <fieldset className="text-center border rounded-xl p-4 mb-16" id="passenger-overview">
      <legend className="text-black mx-auto px-3">
        نگاه کلی به وضعیت مسافران در استان &nbsp;
        {cityTitle}
      </legend>
...
export default OverviewPassengerStatusCardProvince;
