...
const OverviewPassengerStatusCard: React.FC<{}> = () => {
  ...
  const getPcrResult = async (): Promise<any> => {
    setPcrLoading(true);
    try {
      const res = await passengerService.passengerTestResult({}, {cancelToken: cancelToken.token});
      if (res.status === 200) {
  ...
  const getTripCount = async () => {
    setTripLoading(true);
    try {
      const {data} = await hcsService.tripsCount({}, {cancelToken: cancelToken.token});
