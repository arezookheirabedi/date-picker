...
interface OverviewPublicHealthOfInspectedUnitsProps {
    cityTitle: any
}
  
const OverviewPublicHealthOfInspectedUnits: React.FC<OverviewPublicHealthOfInspectedUnitsProps> = ({cityTitle}) => {
    
    const {list: dataset} = useGetOverviewPublicHealthOfInspectedUnits(true);

    return (
        <fieldset className="text-center border rounded-xl p-4 w-1/2">
            <legend className="text-black mx-auto px-3">
            نگاه کلی به بهداشت عمومی واحدهای بازرسی شده استان {cityTitle}
            </legend>
