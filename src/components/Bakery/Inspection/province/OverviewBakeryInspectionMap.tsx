...
const OverviewBakeryInspectionMap: React.FC<OverviewMapProps> = (
  {cityTitle, sideCityStatus, selectDefault, destinationId}
  ) => {
  ...
  const [options, setOptions] = useState({
    chart: {
      map,
      height: '70%',
      className: 'bakery-inspection-map-chart',
      margin: [0, 0, 50, 0],
    },
  ...
  useEffect(() => {
    ...
    if (detectResource) {
      const data = chartRef?.current?.chart.get('inspection').data;
  ...
  const [hasProvinceResources, resources] = useHasProvinceResource() as any;
  ...
  return (
    <fieldset className="text-center border rounded-xl p-4">
      ...
      نگاه کلی به لیست بازرسان استان{' '}
...
export default OverviewBakeryInspectionMap;
