...
const OverviewCategoriesProvince: React.FC<OverviewCategoriesProvinceProps> = ({cityTitle}) => {
  ...
  async function getOverviewByCategory(params: any , province : any) {
    setLoading(true);
    try {
      const {data} = await recruitmentServices.membersTagBased({...params,province});
  ...
  useEffect(() => {
    ...
    if (existsCity) {
      idSetTimeOut = setTimeout(() => {
        getOverviewByCategory(query, provinceName);
      }, 500);
    } else {
